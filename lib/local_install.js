...
async function installOne(parentDir, childPkg, ancestors, options, context) {
  if (!(await needInstallV2(parentDir, childPkg, ancestors, options, context))) {
    ...
    // check resolutions
    await checkChildResolutions(parentDir, childPkg, ancestors, options, context);
    return;
  }
  const res = await install(parentDir, childPkg, ancestors, options, context);
  options.progresses.finishedInstallTasks++;
  if (res) {
    options.console.info(
      '[%s/%s] %s %s at %s',
...
async function needInstallV2(parentDir, childPkg, ancestors, options, context) {
  // always install if not install from package.json
  if (!options.installRoot) return true;
  const isRoot = ancestors.length === 0;
  // TODO 除了 resolution 是否还要判断 flatten mode
  const resolutionPkg = options.resolution(childPkg, ancestors, context.nested);
  // 命中 resolution
  // 判断当前实际存在的包是否与 resolution 匹配
  const realPkg = await utils.readJSON(path.join(parentDir, 'node_modules', childPkg.name, 'package.json'));

  try {
    if (realPkg.name && realPkg.version && childPkg.version) {
      if (semver.validRange(resolutionPkg.version, true) && semver.satisfies(realPkg.version, resolutionPkg.version)) {
        return false;
      }
    }
  } catch (err) {
    // ignore, maybe pkg.version invalid
    debug('[%s@%s] version invalid: %s', realPkg.name, realPkg.version, err);
  }
  // clean up
  if (childPkg.name) {
...
async function needInstall(parentDir, childPkg, ancestors, options) {
  // always install if not install from package.json
  if (!options.installRoot) return true;
...
async function checkChildResolutions(parentDir, childPkg, ancestors, options, context) {
  // 命中了缓存，直接返回
  const realPkg = await utils.readJSON(path.join(parentDir, 'node_modules', childPkg.name, 'package.json'));
  // TODO 研究下 option.cache 的 key 设计，和 install.js 保持一致
  const key = `resolutionCheck:${realPkg.name}@${realPkg.version}`;
  ...
  const pkgDir = utils.getPackageStorePath(options.storeDir, realPkg);
  const deps = dependencies(realPkg, options, context.nested);
  // 确认下 peerDependencies 和 bundledDependencies
  const needPkgs = deps.prod;
  if (needPkgs.length > 0) {
    const mapper = async childPkg => {
      await installOne(pkgDir, childPkg, ancestors.concat({
